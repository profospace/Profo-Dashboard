...
import { getBuildingProperties, disconnectPropertiesFromBuilding } from '../../../utils/Building/api2';

const DisconnectPropertiesFromBuildingPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // Fetch properties connected to this building
                const propertiesData = await getBuildingProperties(buildingId);
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedProperties.length === 0) {
            setError('Please select at least one property to disconnect');
            return;
        }
        ...
        try {
            await disconnectPropertiesFromBuilding(buildingId, { propertyIds: selectedProperties });
            setSuccess(`${selectedProperties.length} properties successfully disconnected from the building.`);

            // Remove disconnected properties from the list
            setProperties(properties.filter(property => !selectedProperties.includes(property._id)));

            // Clear selection
            setSelectedProperties([]);

            // Redirect after a short delay if all properties were disconnected
            if (selectedProperties.length === properties.length) {
                setTimeout(() => {
            ...
            console.error('Error disconnecting properties:', err);
            setError('Failed to disconnect properties. Please try again.');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="mx-auto max-w-7xl">
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900">Disconnect Properties from Building</h1>
                {building && (
            ...
            <div className="bg-white shadow-md rounded-lg overflow-hidden">
                <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                    ...
                    <div className="flex space-x-2">
                        ...
                        >
                            {selectedProperties.length === properties.length && properties.length > 0 ? 'Deselect All' : 'Select All'}
                        </button>
                        <button
                            type="button"
                ...
                {properties.length === 0 ? (
                    <div className="p-8 text-center">
                        ...
                        <h3 className="mt-2 text-sm font-medium text-gray-900">No connected properties found</h3>
                        <p className="mt-1 text-sm text-gray-500">This building has no properties connected to it.</p>
                        <div className="mt-4">
                            <button
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {properties.map((property) => (
                                    <tr key={property._id} className={`${selectedProperties.includes(property._id) ? 'bg-orange-50' : ''} hover:bg-gray-50`}>
                ...
                <div className="bg-gray-50 px-4 py-3 flex justify-end">
                    ...
                    <button
                        ...
                        className={`${submitting || selectedProperties.length === 0
                                ? 'bg-orange-300 cursor-not-allowed'
                                : 'bg-orange-600 hover:bg-orange-700'
                            } py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500`}
                    >
                        {submitting ? 'Disconnecting...' : 'Disconnect Selected Properties'}
                    </button>
                </div>
...
export default DisconnectPropertiesFromBuildingPage;