...
const ConnectBuilderProjectsPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const projectsResponse = await fetch(`${base_url}/api/builder/project/connection/projects/unassigned`);
                ...
                setUnassignedProjects(projectsData.data || []);
            } catch (err) {
    ...
    const handleSelectAll = () => {
        if (selectedProjects.length === unassignedProjects.length) {
            setSelectedProjects([]);
        } else {
            setSelectedProjects(unassignedProjects.map(project => project._id));
        }
    };

    const handleSubmit = async () => {
        if (selectedProjects.length === 0) {
            setError('Please select at least one project to connect');
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`${base_url}/api/builder/project/connection/builders/connect-projects`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                throw new Error(data.message || 'Failed to connect projects');
            }

            setSuccess(`Successfully connected ${selectedProjects.length} projects to ${builder.name}`);
            setSelectedProjects([]);

            // Remove connected projects from the list
            setUnassignedProjects(prev =>
                prev.filter(project => !selectedProjects.includes(project._id))
            );
        } catch (err) {
            console.error('Error connecting projects:', err);
            setError(err.message || 'Failed to connect projects');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <div className="flex items-center mb-6">
                ...
                <h1 className="text-2xl font-bold">Connect Projects to Builder</h1>
            </div>
            ...
            <div className="bg-white shadow-md rounded-lg p-6 mb-6">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div>
                        ...
                        >
                            {selectedProjects.length === unassignedProjects.length ? 'Deselect All' : 'Select All'}
                        </button>
                        <button
                        ...
                        >
                            ...
                            {submitting ? 'Connecting...' : 'Connect Selected Projects'}
                        </button>
                    </div>
                </div>
                        ...
                        <table className="min-w-full bg-white rounded-lg overflow-hidden">
                            ...
                            <tbody>
                                {unassignedProjects.map(project => (
                                    <tr
                                        key={project._id}
                                        className={`border-b hover:bg-gray-50 ${selectedProjects.includes(project._id) ? 'bg-blue-50' : ''
                                            }`}
                                    >
...
export default ConnectBuilderProjectsPage;