...
const ConnectBuilderPropertiesPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const propertiesResponse = await fetch(`${base_url}/api/builder/property/connection/properties/unassigned`);
                if (!propertiesResponse.ok) {
                    throw new Error('Failed to fetch unassigned properties');
                }
    ...
    const handleSubmit = async () => {
        if (selectedProperties.length === 0) {
            setMessage({ text: 'Please select at least one property to connect', type: 'error' });
            return;
        }

        setSubmitting(true);
        try {
            const response = await fetch(`${base_url}/api/builder/property/connection/builders/connect-properties`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Failed to connect properties');
            }

            const data = await response.json();
            setMessage({ text: `Successfully connected ${data.modifiedCount} properties to builder`, type: 'success' });
        ...
        <div className="container mx-auto p-4">
            ...
            <div className="bg-white shadow-md rounded-lg p-6">
                <h1 className="text-2xl font-bold text-gray-800 mb-4">
                    Connect Properties to Builder: {builder.name}
                </h1>
                ...
                <div className="mb-4 flex justify-between items-center">
                    <div>
                        <span className="font-semibold">Total unassigned properties:</span> {properties.length}
                    </div>
                    <div>
                ...
                <div className="mt-6 flex justify-end">
                    <button
                        ...
                        className={`bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition flex items-center ${(selectedProperties.length === 0 || submitting) ? 'opacity-50 cursor-not-allowed' : ''
                            }`}
                    >
...
export default ConnectBuilderPropertiesPage;