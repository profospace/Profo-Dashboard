...
const AdEditPage = () => {
    ...
    const { id } = useParams(); // Get the ad index from URL
    ...
    useEffect(() => {
        const fetchAdData = async () => {
            try {
                ...
                // Get the specific ad by index
                const adIndex = parseInt(id);
                if (isNaN(adIndex) || adIndex < 0 || adIndex >= ads.length) {
                    showNotification('Ad not found', 'error');
                    setTimeout(() => navigate('/ads'), 1500);
                    return;
                }

                const adData = ads[adIndex];
                setFormData(adData);

                // Initialize the input fields for comma-separated lists
                if (adData.imagelinks && adData.imagelinks.length > 0) {
                    setImageLinksInput(adData.imagelinks.join(', '));
                }

                if (adData.contact && adData.contact.length > 0) {
                    setContactInput(adData.contact.join(', '));
                }
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isNaN(adIndex) || adIndex < 0 || adIndex >= updatedAds.length) {
                throw new Error('Invalid ad index');
            }

            updatedAds[adIndex] = formData;
    ...
    return (
        <div className="mx-auto ">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">Edit Ad</h1>
                <button
