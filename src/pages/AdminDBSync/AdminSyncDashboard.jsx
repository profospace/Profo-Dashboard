...
const AdminSyncDashboard = () => {
    ...
    // Fetch sync status if syncId exists and syncing is true
    useEffect(() => {
        let interval = null;
        ...
        // Clear interval when component unmounts or when syncing is complete
        return () => {
    ...
    const checkSyncStatus = async (id) => {
        try {
            ...
            if (!response.data.status.inProgress) {
                setSyncing(false);
                setSyncId(null); // Clear syncId to stop polling
                if (response.data.status.errors?.length > 0) {
                    setError(`Sync completed with ${response.data.status.errors.length} errors.`);
                } else {
                    setSuccess('Sync completed successfully!');
