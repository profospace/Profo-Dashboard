...
// //     // Handle gallery image changes
// //     const handleGalleryChange = (changes) => {
// //         setGalleryChanges(changes);
// //         setFormTouched(true);
// //     };
...
// //         // Add gallery information for editing
// //         preparedData.galleryUpdates = {
// //             retainedImages: galleryChanges.retainedImages,
// //             removedImages: galleryChanges.removedImages
// //         };

// //         return preparedData;
// //     }, [formData, latitude, longitude, flatsDetails, galleryChanges]);
...
// //         if (validateForm()) {
// //             const preparedData = prepareFormData();
// //             // Pass both the form data and the new image files
// //             onSubmit(preparedData, galleryChanges.newImages);
// //         } else {
// //             // Find the first error and switch to its tab
// //             const errorFields = Object.keys(errors);
// //             if (errorFields.length > 0) {
...
const BuildingForm = ({
    ...
    // Check if this is a new building (no initialData with _id)
    const isNewBuilding = !initialData._id && Object.keys(initialData).length === 0;
    ...
    // Handle location change from the map component
    const handleLocationChange = (lat, lng) => {
        // console.log('Location changed:', { lat, lng, types: typeof lat, typeof lng });

        // Ensure we're storing numbers, not strings
        const numLat = lat === '' ? '' : Number(lat);
        const numLng = lng === '' ? '' : Number(lng);
    ...
    const prepareFormData = useCallback(() => {
        ...
        if (latitude && longitude) {
            ...
            // Also add separate lat/lng for backend compatibility
            preparedData.latitude = Number(latitude);
            preparedData.longitude = Number(longitude);
        }
        ...
        return preparedData;
    }, [formData, latitude, longitude, flatsDetails, galleryChanges, isNewBuilding]);
    ...
    const handleSubmit = (e) => {
        ...
        if (validateForm()) {
            const preparedData = prepareFormData();
            // Pass both the form data and the new image files
            const imagesToSubmit = isNewBuilding ? galleryChanges.newImages : galleryChanges.newImages;
            onSubmit(preparedData, imagesToSubmit);
        } else {
            // Find the first error and switch to its tab
            const errorFields = Object.keys(errors);
            if (errorFields.length > 0) {
                ...
                {activeTab === 'location' && (
                    <div className="space-y-6">
                        <GoogleMapComponent
                            ...
                            onLocationChange={handleLocationChange}
                            height="400px"
                            autoFetchLocation={isNewBuilding}
                        />
