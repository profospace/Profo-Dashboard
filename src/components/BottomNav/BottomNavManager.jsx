...
import React, { useState, useEffect } from 'react';
import { Plus, Settings, Eye, EyeOff, Upload, Trash2, Save, RefreshCw, Edit, GripVertical } from 'lucide-react';
import IconCard from "./IconCard";
import IconEditor from './IconEditor';
import PreviewModal from './PreviewModal';
import { base_url } from '../../../utils/base_url';
...
const BottomNavManager = () => {
    ...
    const fetchIcons = async () => {
        try {
            ...
            const response = await axios.get(`${API_BASE_URL}/api/bottom-nav-icons`, {
                ...getAuthConfig(),
            ...
            if (response.data.success) {
                ...
                const iconsData = response.data.data?.icons || [];
                setIcons(iconsData);
                console.log('Fetched icons:', iconsData);
            } else {
                throw new Error(response.data.message || 'Failed to fetch icons');
            }
        } catch (error) {
            console.error('Error fetching icons:', error);
            setError('Failed to load icons: ' + (error.response?.data?.message || error.message));
    ...
    const handleDeleteIcon = async (iconId) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                throw new Error(response.data.message || 'Failed to delete icon');
            }
        } catch (error) {
            console.error('Error deleting icon:', error);
            const errorMessage = error.response?.data?.message || error.message;
            setError('Failed to delete icon: ' + errorMessage);
            alert('Failed to delete icon: ' + errorMessage);
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleToggleEnabled = async (icon) => {
        try {
            ...
            const response = await axios.put(`${API_BASE_URL}/api/bottom-nav-icons/${icon.id}`, formData, {
                ...getAuthConfig(),
                headers: {
            ...
            if (response.data.success) {
                ...
                throw new Error(response.data.message || 'Failed to update icon');
            }
        } catch (error) {
            console.error('Error toggling icon:', error);
            const errorMessage = error.response?.data?.message || error.message;
            setError('Failed to update icon: ' + errorMessage);
    ...
    const handleSaveIcon = async (formData) => {
        try {
            ...
            if (editingIcon) {
                // Update existing icon
                response = await axios.put(`${API_BASE_URL}/api/bottom-nav-icons/${editingIcon.id}`, formData, {
                    ...getAuthConfig(),
                    headers: {
                        ...getAuthConfig().headers,
                        'Content-Type': 'multipart/form-data'
                ...
                // Create new icon
                response = await axios.post(`${API_BASE_URL}/api/bottom-nav-icons`, formData, {
                    ...getAuthConfig(),
                    headers: {
                        ...getAuthConfig().headers,
                        'Content-Type': 'multipart/form-data'
            ...
            if (response.data.success) {
                ...
                throw new Error(response.data.message || 'Failed to save icon');
            }
        } catch (error) {
            console.error('Error saving icon:', error);
            const errorMessage = error.response?.data?.message || error.message;
            setError('Failed to save icon: ' + errorMessage);
            alert('Failed to save icon: ' + errorMessage);
        } finally {
            setSaving(false);
        }
    };
    ...
    const enabledIcons = icons.filter(icon => icon.isEnabled).length;
    const disabledIcons = totalIcons - enabledIcons;
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white rounded-lg shadow p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <p className="text-sm font-medium text-gray-500">Total Icons</p>
                                <p className="text-2xl font-semibold text-gray-900">{totalIcons}</p>
                            </div>
                        </div>
                    </div>
                    <div className="bg-white rounded-lg shadow p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <p className="text-sm font-medium text-gray-500">Enabled</p>
                                <p className="text-2xl font-semibold text-gray-900">{enabledIcons}</p>
                            </div>
                        </div>
                    </div>
                    <div className="bg-white rounded-lg shadow p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <p className="text-sm font-medium text-gray-500">Disabled</p>
                                <p className="text-2xl font-semibold text-gray-900">{disabledIcons}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow">
                    <div className="px-6 py-4 border-b border-gray-200">
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                            {totalIcons} icon{totalIcons !== 1 ? 's' : ''} configured
                        </p>
                    </div>
